import logging, uuid, requests, qrcode, io
...
async def receive_utr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID and context.user_data.get("changing_amount"):
        ...
        try:
            ...
            if order_id in pending_payments:
                ...
                kb = [[InlineKeyboardButton("UTR Number Daalo", callback_data=f"utr_{order_id}")]]
                await context.bot.send_photo(chat_id=user_id, photo=qr_image, caption=f"Amount Update Ho Gaya!\n\nNaya Amount: Rs{new_amount}/-\nUPI ID: {UPI_ID}\nOrder: {order_id}\n\nNaya QR scan karo aur pay karo!", reply_markup=InlineKeyboardMarkup(kb))
    ...
    try:
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"Naya Payment Request!\n\nUser: {user.first_name}\nUser ID: {user.id}\nOrder: {order_id}\nAmount: Rs{amount}/-\nUTR: {text}\n\nAction lo:", reply_markup=InlineKeyboardMarkup(kb))
