...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # payment_link ya payment_session_id se link banao
    payment_link = None
    if cf_order:
        payment_link = cf_order.get("payment_link")
        if not payment_link and cf_order.get("payment_session_id"):
            payment_link = f"https://payments.cashfree.com/order/#?order_token={cf_order['payment_session_id']}"
        if not payment_link and cf_order.get("cf_order_id"):
            payment_link = f"https://payments.cashfree.com/order/#?order_id={order_id}&appId={CASHFREE_APP_ID}"
    ...
    if payment_link:
        ...
        # Fallback: UPI QR (agar Cashfree payment link na mile)
        logger.warning(f"Cashfree payment link nahi mila, falling back to UPI QR. Response: {cf_order}")
