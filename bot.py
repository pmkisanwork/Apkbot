...
# Settings file - price aur app link save hoga
...
async def utr_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        ...
        f"━━━━━━━━━━━━━━━━━━━\n\n"
        f"🔖 Order: `{order_id}`\n\n"
        f"ℹ️ UTR/Transaction ID kahan milega?\n"
        ...
        if action == "change_price":
            try:
                ...
                await update.message.reply_text(
                    ...
                    f"Ab se sabko ₹{new_price}/- ka QR milega!",
                    parse_mode="Markdown"
                )
            except:
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_callback, pattern="^admin_|^approve_|^reject_|^changeamt_"))
